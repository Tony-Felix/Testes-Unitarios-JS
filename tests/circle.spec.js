...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  ...
  it('Testa se circle retorna undefined, caso o parâmetro passado não seja um número.', () => {
    const parametro = '2';
    expect(circle(parametro)).toBe(undefined);
  });

  it('Testa se circle retorna um objeto.', () => {
    const retorno = circle(2);
    expect(retorno).toBeInstanceOf(Object);
  });

  it('Testa se o objeto retornado possui 3 propriedades.', () => {
    const objeto = circle(2);
    expect(objeto).toHaveProperty('radius', 'area', 'circumference');
  });
  ...
  it('retorna uma `key` com `value` igual à circunferência para um círculo de raio 2.', () => {
    const test = circle(2);
    expect(test.circumference).toBe(12.56);
  });

  it('Testa se dentro do objeto retornado, a função retorna uma `key` com `value` igual à área correta para um círculo de raio 3.', () => {
    const test = circle(3);
    expect(test.area).toBe(28.259999999999998);
  });
